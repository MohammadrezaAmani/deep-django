...
from pika import amqp_object, data
...
class Connection(amqp_object.Class):
    ...
    class Start(amqp_object.Method):
        ...
        def encode(self):
            ...
            assert isinstance(
                self.mechanisms, str_or_bytes
            ), "A non-string value was supplied for self.mechanisms"
            value = (
                self.mechanisms.encode("utf-8")
                if isinstance(self.mechanisms, unicode_type)
                else self.mechanisms
            )
            pieces.append(struct.pack(">I", len(value)))
            pieces.append(value)
            assert isinstance(
                self.locales, str_or_bytes
            ), "A non-string value was supplied for self.locales"
            value = (
                self.locales.encode("utf-8")
                if isinstance(self.locales, unicode_type)
                else self.locales
            )
            pieces.append(struct.pack(">I", len(value)))
            pieces.append(value)
    ...
    class Close(amqp_object.Method):
        ...
        def __init__(
            self, reply_code=None, reply_text="", class_id=None, method_id=None
        ):
    ...
    class UpdateSecret(amqp_object.Method):
        ...
        def encode(self):
            pieces = list()
            assert isinstance(
                self.new_secret, str_or_bytes
            ), "A non-string value was supplied for self.new_secret"
            value = (
                self.new_secret.encode("utf-8")
                if isinstance(self.new_secret, unicode_type)
                else self.new_secret
            )
            pieces.append(struct.pack(">I", len(value)))
            pieces.append(value)
            assert isinstance(
...
class Queue(amqp_object.Class):
    ...
    class Delete(amqp_object.Method):
        ...
        def __init__(
            self, ticket=0, queue="", if_unused=False, if_empty=False, nowait=False
        ):
            self.ticket = ticket
            self.queue = queue
            self.if_unused = if_unused
    ...
    class Unbind(amqp_object.Method):
        ...
        def __init__(
            self, ticket=0, queue="", exchange=None, routing_key="", arguments=None
        ):
            self.ticket = ticket
            self.queue = queue
            self.exchange = exchange
...
class Basic(amqp_object.Class):
    ...
    class Consume(amqp_object.Method):
        ...
        def encode(self):
            ...
            data.encode_short_string(pieces, self.queue)
            assert isinstance(
                self.consumer_tag, str_or_bytes
    ...
    class Return(amqp_object.Method):
        ...
        def __init__(
            self, reply_code=None, reply_text="", exchange=None, routing_key=None
        ):
            self.reply_code = reply_code
            self.reply_text = reply_text
            self.exchange = exchange
...
class BasicProperties(amqp_object.Properties):
    ...
    def encode(self):
        ...
        if self.content_type is not None:
            ...
            assert isinstance(
                self.content_type, str_or_bytes
        ...
        if self.content_encoding is not None:
            ...
            assert isinstance(
                self.content_encoding, str_or_bytes
        ...
        if self.correlation_id is not None:
            ...
            assert isinstance(
                self.correlation_id, str_or_bytes
